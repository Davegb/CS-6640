...
def train(env, config, outputs=None):
  ...
  def per_episode(ep):
    ...
    #score = float(ep['reward'][-1].astype(np.float64).sum())
    score = float(ep['reward'][-1].astype(np.float64).sum()) / length
    posx = max(ep['posx'])
    with open("E:\logdir\qwop\dreamerv2\\vellegs\qwop.log", "a") as f:
        f.write(f"{time.time()},{length},{score},{posx}\n")
    print(f'Episode has {length} steps and return {score:.1f}.')
    logger.scalar('return', score)
    logger.scalar('length', length)
    logger.scalar('posx', posx[0])
